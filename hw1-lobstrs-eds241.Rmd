...
spiny_counts <- tidydata |> 
  ...
  summarize(
    counts = n(),  # Total number of lobsters per site-year-transect
    mean_size = mean(size_mm, na.rm = TRUE),  # Mean size, excluding NA
    .groups = 'drop'
  ) |> 
  mutate(
...
plot1 <- spiny_counts |>
  ...
  geom_vline(data = spiny_counts |> group_by(site) |> 
              ...
              aes(xintercept = mean_count), linetype = "dashed", linewidth = 1, color = "red") +
  labs(x = "Lobster Counts", y = "Density", fill = "Reef Site", 
...
plot2 <- spiny_counts |>
  ...
  scale_fill_discrete(labels = c("MPA" = "Treatement Site", "non_MPA" = "Control Site")) +
  labs(x = "Site Type", y = "Lobster Counts", fill = "Site Type",
...
plot4 <- spiny_counts |>
  ...
  geom_vline(data = spiny_counts |> 
              ...
              aes(xintercept = mean_size_val), linetype = "dashed", linewidth = 1, color = "red") +
  scale_fill_manual(values = c("MPA" = "steelblue", "non_MPA" = "darkorange"), 
                    labels = c("MPA" = "Treatment", "non_MPA" = "Control"), 
                    name = "Site Type") +
  labs(x = "Mean Size (mm)", y = "Frequency", fill = "Site Type",
